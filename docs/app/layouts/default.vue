...
const links = computed(() => {
  if (route.path.startsWith('/docs/guide')) {
    return navigation.value.find((item: any) => item.path === '/docs/guide')?.children || []
  }

  return navigation.value.find((item: any) => item.path === '/docs/reference')?.children || []
})
...
const data = ref<any[]>([
  {
    path: '/docs/guide',
    tabs_icon: 'solar:document-linear',
    tabs_title: 'Guide',
  },
  {
    path: '/docs/reference',
    tabs_icon: 'solar:code-square-linear',
    tabs_title: 'Reference',
  },
])
                ...
                >
                  ...
                  >
                    <Icon
                      ...
                      class="h-5 w-5"
                    />
                    <span>{{ child.title || child.path }}</span>
                  </NuxtLink>
                    ...
                    <li v-for="subchild in child?.children" :key="subchild.path">
                      ...
                      >
                        <Icon
                          ...
                          class="h-5 w-5"
                        />
                        <span>{{ subchild.title || subchild.path }}</span>
                      </NuxtLink>
                    </li>
                  </ul>
                </li>
        ...
        <div class="fixed start-0 top-0 z-[5] w-full border-b border-muted-200 bg-white dark:border-muted-800 dark:bg-muted-900 lg:static ltablet:static">
          <div class="mx-auto max-w-3xl">
            <div class="flex h-14 items-center gap-8 px-6 md:gap-10">
              ...
              >
                <Icon :name="tab.tabs_icon" class="h-5 w-5" />
                <span class="hidden text-sm font-medium text-muted-800 dark:text-muted-100 md:inline">{{ tab.tabs_title }}</span>
              </NuxtLink>
